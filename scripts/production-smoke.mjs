import {
  assert,
  fetchTimed,
  productionHealthTimeoutMs,
  productionTargets,
  retryReadOnlyOperation,
  sitemapIndexUrls,
  sitemapIncludesCanonicalPath,
  validateGoogleOAuthRedirect,
} from "./lib/production-monitoring.mjs";

const jsonCheck = async (
  url,
  name,
  validate = () => true,
  requestOptions = {},
) => {
  const { response, durationMs } = await fetchTimed(url, {
    headers: { Accept: "application/json" },
    ...requestOptions,
  });
  assert(response.status === 200, name + " returned " + response.status);
  assert(
    response.headers.get("content-type")?.includes("application/json"),
    name + " did not return JSON",
  );
  const body = await response.json();
  assert(body?.success === true, name + " returned an unsuccessful payload");
  assert(validate(body), name + " returned an invalid response contract");
  return { body, durationMs, httpStatus: response.status };
};

const readOnlyCheck = (name, operation) =>
  retryReadOnlyOperation(operation, {
    onRetry: (error, attempt) => {
      process.stderr.write(
        `[production-smoke] ${name} attempt ${attempt} failed: ${error.message}; retrying\n`,
      );
    },
  });

const main = async () => {
  assert(
    process.env.ALLOW_REMOTE_PRODUCTION_SMOKE === "true",
    "Set ALLOW_REMOTE_PRODUCTION_SMOKE=true after confirming production targets",
  );
  const targets = productionTargets();
  const checks = [];
  const record = (name, result) => {
    checks.push({ name, status: "passed", ...result });
  };

  const documentResult = await readOnlyCheck("client document", async () => {
    const result = await fetchTimed(targets.clientOrigin);
    assert(
      result.response.status === 200,
      "Client returned " + result.response.status,
    );
    assert(
      result.response.headers.get("content-type")?.includes("text/html"),
      "Client did not return HTML",
    );
    const html = await result.response.text();
    assert(html.includes('id="root"'), "Client HTML is missing the React root");
    return result;
  });
  record("client document", {
    httpStatus: documentResult.response.status,
    durationMs: documentResult.durationMs,
  });

  const manifestResult = await readOnlyCheck("web manifest", async () => {
    const result = await fetchTimed(
      targets.clientOrigin + "/favicon/site.webmanifest",
    );
    assert(
      result.response.status === 200,
      "Manifest returned " + result.response.status,
    );
    assert(
      result.response.headers.get("content-type")?.includes("manifest+json"),
      "Manifest MIME type is invalid",
    );
    const manifest = await result.response.json();
    assert(manifest.name === "HTCOACHING", "Manifest name is invalid");
    assert(
      Array.isArray(manifest.icons) && manifest.icons.length >= 2,
      "Manifest icons are missing",
    );
    return result;
  });
  record("web manifest", {
    httpStatus: manifestResult.response.status,
    durationMs: manifestResult.durationMs,
  });

  for (const [name, path] of [
    ["api liveness", "/api/ops/health/live"],
    ["api readiness", "/api/ops/health/ready"],
  ]) {
    const result = await readOnlyCheck(
      name,
      (attempt) =>
        jsonCheck(targets.apiOrigin + path, name, () => true, {
          timeoutMs: productionHealthTimeoutMs(attempt),
        }),
    );
    record(name, { httpStatus: 200, durationMs: result.durationMs });
  }

  const oauthResult = await readOnlyCheck("Google OAuth topology", async () => {
    const result = await fetchTimed(
      targets.apiOrigin +
        "/api/auth/google?client_url=" +
        encodeURIComponent(targets.clientOrigin),
    );
    assert(
      result.response.status >= 300 && result.response.status < 400,
      "Google OAuth start returned " + result.response.status,
    );
    validateGoogleOAuthRedirect(result.response.headers.get("location"));
    return result;
  });
  record("Google OAuth topology", {
    httpStatus: oauthResult.response.status,
    durationMs: oauthResult.durationMs,
  });
  const blog = await readOnlyCheck("blog public API", () =>
    jsonCheck(
      targets.apiOrigin + "/api/blog?limit=1",
      "blog public API",
      (body) => Array.isArray(body.data),
    ),
  );
  record("blog public API", { httpStatus: 200, durationMs: blog.durationMs });

  const recipes = await readOnlyCheck("recipe public API", () =>
    jsonCheck(
      targets.apiOrigin + "/api/recipes?limit=1",
      "recipe public API",
      (body) =>
        Array.isArray(body.data) &&
        Number.isInteger(body.pagination?.total) &&
        body.pagination?.limit === 1,
    ),
  );
  record("recipe public API", {
    httpStatus: 200,
    durationMs: recipes.durationMs,
  });

  const taxonomy = await readOnlyCheck("recipe taxonomy API", () =>
    jsonCheck(
      targets.apiOrigin + "/api/recipes/categories",
      "recipe taxonomy API",
      (body) => Array.isArray(body.data),
    ),
  );
  record("recipe taxonomy API", {
    httpStatus: 200,
    durationMs: taxonomy.durationMs,
  });

  const firstBlog = blog.body.data[0];
  if (firstBlog?.slug) {
    const detail = await readOnlyCheck("blog detail API", () =>
      jsonCheck(
        targets.apiOrigin + "/api/blog/" + encodeURIComponent(firstBlog.slug),
        "blog detail API",
        (body) => body.data?.slug === firstBlog.slug,
      ),
    );
    record("blog detail API", {
      httpStatus: 200,
      durationMs: detail.durationMs,
    });
  }

  const firstRecipe = recipes.body.data[0];
  if (firstRecipe?.slug) {
    const detail = await readOnlyCheck("recipe detail API", () =>
      jsonCheck(
        targets.apiOrigin +
          "/api/recipes/detail/" +
          encodeURIComponent(firstRecipe.slug),
        "recipe detail API",
        (body) => body.data?.slug === firstRecipe.slug,
      ),
    );
    record("recipe detail API", {
      httpStatus: 200,
      durationMs: detail.durationMs,
    });
  }

  const sitemapResult = await readOnlyCheck("dynamic sitemap", async () => {
    const result = await fetchTimed(targets.clientOrigin + "/sitemap.xml");
    assert(
      result.response.status === 200,
      "Sitemap returned " + result.response.status,
    );
    const sitemap = await result.response.text();
    const childSitemapUrls = sitemapIndexUrls(sitemap, targets.clientOrigin);
    const sitemapDocuments = [sitemap];
    let durationMs = result.durationMs;
    if (childSitemapUrls.length > 0) {
      for (const childUrl of childSitemapUrls) {
        const childResult = await fetchTimed(childUrl);
        assert(
          childResult.response.status === 200,
          "Child sitemap returned " + childResult.response.status,
        );
        const childSitemap = await childResult.response.text();
        assert(childSitemap.includes("<urlset"), "Child sitemap XML is invalid");
        sitemapDocuments.push(childSitemap);
        durationMs += childResult.durationMs;
      }
    } else {
      assert(sitemap.includes("<urlset"), "Sitemap XML is invalid");
    }
    const sitemapCorpus = sitemapDocuments.join("\n");
    for (const path of ["/blog", "/cong-thuc-nau-an"]) {
      assert(
        sitemapIncludesCanonicalPath(sitemapCorpus, path),
        "Sitemap is missing " + path,
      );
    }
    for (const [prefix, item] of [
      ["/blog/", firstBlog],
      ["/cong-thuc-nau-an/", firstRecipe],
    ]) {
      if (item?.slug) {
        assert(
          sitemapIncludesCanonicalPath(sitemapCorpus, prefix + item.slug),
          "Sitemap is missing the current " + prefix + " detail route",
        );
      }
    }
    return {
      response: result.response,
      durationMs: Number(durationMs.toFixed(2)),
      childSitemapCount: childSitemapUrls.length,
    };
  });
  record("dynamic sitemap", {
    httpStatus: sitemapResult.response.status,
    durationMs: sitemapResult.durationMs,
    childSitemapCount: sitemapResult.childSitemapCount,
  });

  process.stdout.write(
    JSON.stringify(
      { success: true, checkedAt: new Date().toISOString(), checks },
      null,
      2,
    ) + "\n",
  );
};

main().catch((error) => {
  process.stderr.write(
    JSON.stringify({ success: false, error: error.message }) + "\n",
  );
  process.exitCode = 1;
});
