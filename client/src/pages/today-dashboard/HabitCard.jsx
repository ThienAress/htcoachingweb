...
export const HabitListItem = ({
  ...
  return (
    <li className="rounded-xl border border-slate-700 p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <button
          type="button"
          onClick={() => onComplete(habit, "completed")}
          disabled={actionDisabled}
          aria-describedby={!habit.scheduledToday ? scheduleMessageId : undefined}
          aria-pressed={completion?.status === "completed"}
          className={`inline-flex min-h-11 items-center gap-2 rounded-lg border border-emerald-700 px-3 py-2 text-sm font-semibold text-emerald-200 hover:bg-emerald-950/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-300 disabled:cursor-not-allowed disabled:opacity-40 ${
        ...
        <button
          type="button"
          onClick={() => onComplete(habit, "skipped")}
          disabled={actionDisabled}
          aria-describedby={!habit.scheduledToday ? scheduleMessageId : undefined}
          aria-pressed={completion?.status === "skipped"}
          className={`inline-flex min-h-11 items-center gap-2 rounded-lg border px-3 py-2 text-sm font-semibold text-slate-300 hover:bg-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-300 disabled:cursor-not-allowed disabled:opacity-40 ${
    ...
    >
      ...
      {habitsQuery.isLoading ? (
        ...
        <p className="mt-4 text-sm text-slate-400">
          Không có thói quen áp dụng cho ngày này.
        </p>
      ) : (
        <ul className="mt-4 grid gap-3 lg:grid-cols-2">
          {activeInRange.map((habit) => {
