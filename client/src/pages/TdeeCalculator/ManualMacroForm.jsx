...
const ManualMacroForm = ({ setShowLoginModal }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (!p && !c && !f) {
      setError("Vui lòng nhập ít nhất một chỉ số macro.");
      return;
    }
  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-white/10 w-full max-w-4xl mx-auto animate-fade-in-up">
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl font-bold text-white mb-2 uppercase">Nhập Macro Thủ Công</h2>
        <p className="text-gray-400">
          Nhập các chỉ số Đạm, Tinh bột, Chất béo của bạn. Hệ thống sẽ tự động tính lượng Calo và đưa bạn đến thẳng Gợi ý thực đơn.
        </p>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-gray-800/50 rounded-xl p-6 border border-gray-700 flex flex-col items-center justify-center space-y-2">
          <span className="text-gray-400 font-medium flex items-center gap-2">
            <Activity className="w-4 h-4" /> Tổng năng lượng (Calories)
          </span>
