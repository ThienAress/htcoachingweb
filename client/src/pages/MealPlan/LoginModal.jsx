import React from "react";
import { X } from "lucide-react";
import { useTranslation } from "react-i18next";
import logo from "../../assets/images/logo/logo.svg";
import { getServerBaseUrl } from "../../utils/mediaUrl";

const LoginModal = ({ isOpen, onClose }) => {
  const { t } = useTranslation("mealplan");

  if (!isOpen) return null;

  const handleGoogleLogin = () => {
    // Lưu trang hiện tại để sau login quay lại
    localStorage.setItem("redirectAfterLogin", "/mealplan");
    const apiUrl =
      import.meta.env.VITE_API_URL ||
      (import.meta.env.DEV ? "http://localhost:5000/api" : "");
    const baseUrl = getServerBaseUrl(apiUrl);
    if (!baseUrl) throw new Error("Missing VITE_API_URL");
    window.location.href = `${baseUrl}/api/auth/google?client_url=${encodeURIComponent(window.location.origin)}`;
  };

  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
      {/* Overlay */}
      <div
        className="absolute inset-0 bg-black/80 backdrop-blur-sm"
        onClick={onClose}
      ></div>

      {/* Modal Content */}
      <div className="relative bg-gray-900 border border-gray-800 rounded-2xl shadow-2xl p-8 w-full max-w-md text-center z-10">
        <button
          aria-label={t("login_modal.close")}
          onClick={onClose}
          className="absolute top-3 right-3 flex min-h-11 min-w-11 items-center justify-center rounded-lg text-gray-400 hover:bg-white/5 hover:text-white transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"
          type="button"
        >
          <X size={24} />
        </button>

        {/* Logo */}
        <img src={logo} alt="HT Coaching" className="h-14 mx-auto mb-5" />

        {/* Tiêu đề */}
        <h2 className="text-3xl font-black mb-2 tracking-wider uppercase">
          <span className="text-primary font-bebas-neue">
            {t("login_modal.title")}
          </span>
        </h2>

        {/* Thông báo */}
        <p className="text-gray-300 text-sm font-medium mb-8 leading-relaxed">
          {t("login_modal.description")}
        </p>

        {/* Nút đăng nhập Google */}
        <button
          onClick={handleGoogleLogin}
          className="w-full min-h-12 flex items-center justify-center gap-3 bg-primary hover:bg-orange-500 text-white font-bold py-3 px-4 rounded-lg transition-colors shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-300"
          type="button"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="white">
            <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
            <path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
            <path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
          </svg>
          {t("login_modal.google")}
        </button>
      </div>
    </div>
  );
};

export default LoginModal;
