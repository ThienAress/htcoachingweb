...
const CONFIRMATION_VARIABLE = "CONFIRM_EXERCISE_REVIEW_INDEX_MIGRATION";
...
export const getExerciseReviewIndexContracts = () =>
  ExerciseReview.schema
    .indexes()
...
const listIndexes = async () => {
  try {
    return await ExerciseReview.collection.listIndexes().toArray();
  } catch (error) {
...
const duplicateCount = async (contract) => {
  ...
  try {
    const [result] = await ExerciseReview.collection
      .aggregate([
        {
          $group: {
...
export const inspectExerciseReviewIndexes = async () => {
  const existing = await listIndexes();
  const reports = [];
  for (const contract of getExerciseReviewIndexContracts()) {
  ...
  if (reports.length !== TARGET_INDEX_NAMES.size) {
    throw new Error("Exercise review index contract manifest is incomplete");
  }
  return reports;
};

export const applyExerciseReviewIndexes = async (reports) => {
  if (
    reports.some(
      ({ duplicateGroupCount, status }) =>
        duplicateGroupCount || status === "name_conflict",
    )
  ) {
    throw new Error("Exercise review index apply blocked by preflight findings");
  }
  const applied = [];
  for (const { contract, status } of reports) {
    ...
    const name = await ExerciseReview.collection.createIndex(
      contract.keys,
      contract.options,
    );
...
export const authorizeExerciseReviewIndexTarget = ({
  args,
  apply,
  env = process.env,
}) => {
  ...
  if (env.APP_ENV !== target) {
    throw new Error("Exercise review index target does not match APP_ENV");
  }
  ...
  if (!uriDatabase || !targetDatabase || uriDatabase !== targetDatabase) {
    throw new Error("Exercise review index database target lock failed");
  }
  if (!apply) return { targetDatabase, valid: true };
  if (!args.has("--confirm-exercise-review-indexes")) {
    throw new Error("Apply requires --confirm-exercise-review-indexes");
  }
...
const main = async () => {
  ...
  const authorization = authorizeExerciseReviewIndexTarget({ args, apply });
  ...
  try {
    ...
    if (
      ...
      throw new Error("Exercise review index preflight blocked");
    }
    const applied = apply ? await applyExerciseReviewIndexes(reports) : [];
    const verification = apply
      ? await inspectExerciseReviewIndexes()
      : reports;
    const success =
    ...
    if (!success) throw new Error("Exercise review index verification failed");
  } finally {
